...
const dragBtns: HTMLElement[] = Array.from(
	listParent.querySelectorAll(`${sels.item} button`)
);
...
const onDrag = (e: TouchEvent | MouseEvent) => {
	...
	const listEls: HTMLElement[] = Array.from(
		listParent.querySelectorAll(sels.item)
	);
